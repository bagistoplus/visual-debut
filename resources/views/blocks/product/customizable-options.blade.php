  ...
  @if ($product->getTypeInstance()->isCustomizable())
    ...
    @if ($options->isNotEmpty())
      <div
        ...
        x-data
        x-product-customizable="@js(['options' => $options, 'basePrice' => $product->getTypeInstance()->getMinimalPrice()])"
      >
        <div class="space-y-4">
              ...
              @switch($option->type)
                ...
                @case('file')
                  ...
                  <input
                    ...
                    name="customizable_options[{{ $option->id }}]"
                    @if ($option->supported_file_extensions) accept="{{ collect(explode(',', $option->supported_file_extensions))->map(fn($ext) => '.' . trim($ext))->implode(',') }}" @endif
  ...
  <div {{ $block->editor_attributes }} class="text-muted">
    <div class="border-b pb-3">
      ...
      <input
        ...
        class="mt-1 w-full"
        disabled
        placeholder="{{ trans('visual-debut::blocks.product-customizable-options.placeholder.enter-value') }}"
      >
    </div>
  </div>
