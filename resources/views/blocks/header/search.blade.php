<div {{ $block->editor_attributes }} id="search-form">
  <div x-data="{ showSearch: false }">
    ...
    >
      @svg($block->settings->search_icon ?? 'lucide-search', ['class' => 'hover:text-primary h-5 w-5 transition-colors'])
    </button>
    ...
    >
      <div class="mx-auto max-w-7xl px-4 py-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          @svg($block->settings->search_icon ?? 'lucide-search', ['class' => 'hover:text-primary absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 transition-colors'])
          ...
          @if (core()->getConfigData('catalog.products.settings.image_search'))
            <x-shop::image-search-button :icon="$block->settings->image_search_icon ?? 'lucide-camera'" class="absolute right-4 top-1/2 flex -translate-y-1/2 transform items-center" />
          @endif
        </form>
      </div>
    </div>
