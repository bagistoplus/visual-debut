...
<div
  ...
  x-data
  x-navigation
  class="hidden h-full items-center sm:flex"
>
  <div class="relative">
      ...
      @foreach ($categories as $category)
        @if ($category->children->isNotEmpty())
          @php
            ...
            $categoryDescription = str(strip_tags($category->description ?? ''))
                ->squish()
                ->limit(150);
          @endphp
            ...
            @if ($hasImage)
              <div class="text-surface-200 box relative flex h-full min-h-64 w-40 flex-shrink-0 items-end overflow-hidden border-none p-4">
                ...
                <div class="relative space-y-1.5">
                  ...
                  <span class="text-surface-200/80 block text-sm">{{ $categoryDescription }}</span>
                </div>
              </div>
            @endif
                ...
                @foreach ($category->children as $subCategory)
                  <div class="min-w-60">
                    <a
                      ...
                      class="hover:bg-surface-alt box block h-full border-none px-3.5 py-3 text-sm"
                    >
                      <span class="text-on-background mb-1 block font-semibold">
                        {{ $subCategory->name }}
                      </span>
                      @if ($subCategory->description)
                        <span class="text-on-background/70 block leading-5">
                          {{ str(strip_tags($subCategory->description ?? ''))->squish()->limit(70) }}
                        </span>
                      @endif
                    </a>
                  </div>
