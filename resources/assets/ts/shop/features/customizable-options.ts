...
interface CustomizableOptionsState {
  ...
  init(): void;
  canHaveMultiplePriceOptions(type: string): boolean;
  validate(scrollToFirstInvalid?: boolean): boolean;
  hasError(optionId: number): boolean;
  scrollToFirstInvalid(): void;
  calculateOptionPrice(option: CustomizableOption): number;
}

type CustomizableOptionsAPI = CustomizableOptionsState;

export default defineComponent<CustomizableOptionsAPI>({
